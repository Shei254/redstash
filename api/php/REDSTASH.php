 ...
 *
 * REDSTASH PHP client SDK.
 */

class REDSTASHException extends Exception
{
}

class REDSTASHTimeoutException extends REDSTASHException
{
}
 ...
 */
class SimpleREDSTASH extends REDSTASH
{
...
// Depricated, use SimpleREDSTASH instead!
class REDSTASH
{
	private $debug = false;
	public $sock = null;
	...
	function __construct($host, $port, $timeout_ms=2000){
		...
		if(!$this->sock){
			throw new REDSTASHException("$errno: $errstr");
		}
	 ...
	 * will not return a REDSTASH_Response object.
	...
	function __call($cmd, $params=array()){
		...
		try{
			if($this->send_req($cmd, $params) === false){
				$resp = new REDSTASH_Response('error', 'send error');
			}else{
				$resp = $this->recv_resp($cmd, $params);
			}
		}catch(REDSTASHException $e){
			if($this->_easy){
				throw $e;
			}else{
				$resp = new REDSTASH_Response('error', $e->getMessage());
			}
		}
	...
	private function recv_resp($cmd, $params){
		$resp = $this->recv();
		if($resp === false){
			return new REDSTASH_Response('error', 'Unknown error');
		}else if(!$resp){
			return new REDSTASH_Response('disconnected', 'Connection closed');
		}
		if($resp[0] == 'noauth'){
			$errmsg = isset($resp[1])? $resp[1] : '';
			return new REDSTASH_Response($resp[0], $errmsg);
		}
		switch($cmd){
			case 'dbsize':
			case 'ping':
			...
			case 'expire':
				if($resp[0] == 'ok'){
					...
					return new REDSTASH_Response($resp[0], $val);
				}else{
					$errmsg = isset($resp[1])? $resp[1] : '';
					return new REDSTASH_Response($resp[0], $errmsg);
				}
			...
			case 'qback':
				if($resp[0] == 'ok'){
					if(count($resp) == 2){
						return new REDSTASH_Response('ok', $resp[1]);
					}else{
						return new REDSTASH_Response('server_error', 'Invalid response');
					}
				}else{
					$errmsg = isset($resp[1])? $resp[1] : '';
					return new REDSTASH_Response($resp[0], $errmsg);
			...
			case 'qpop_back':
				if($resp[0] == 'ok'){
					...
					if($size <= 1){
						...
						return new REDSTASH_Response('ok', $data);
					}
				}else{
					$errmsg = isset($resp[1])? $resp[1] : '';
					return new REDSTASH_Response($resp[0], $errmsg);
			...
			case 'qslice':
				if($resp[0] == 'ok'){
					...
					return new REDSTASH_Response($resp[0], $data);
				}else{
					$errmsg = isset($resp[1])? $resp[1] : '';
					return new REDSTASH_Response($resp[0], $errmsg);
				}
			...
			case 'zexists':
				if($resp[0] == 'ok'){
					if(count($resp) == 2){
						return new REDSTASH_Response('ok', (bool)$resp[1]);
					}else{
						return new REDSTASH_Response('server_error', 'Invalid response');
					}
				}else{
			...
			case 'zpop_back':
				...
				break;
			default:
				return new REDSTASH_Response($resp[0], array_slice($resp, 1));
		}
		return new REDSTASH_Response('error', 'Unknown command: $cmd');
	}
	...
	function send($data){
		...
		try{
			while(true){
				...
				if($ret === false || $ret === 0){
					$this->close();
					throw new REDSTASHException('Connection lost');
				}
				$s = substr($s, $ret);
				if(strlen($s) == 0){
					break;
			...
			$this->close();
			throw new REDSTASHException($e->getMessage());
		}
		return $ret;
	}
			...
			if($ret === null){
				...
				if($data === false || $data === ''){
					if(feof($this->sock)){
						...
						throw new REDSTASHTimeoutException('Connection timeout');
					}
				}
				$this->recv_buf .= $data;
