...
rm -f src/version.h
echo "#ifndef REDSTASH_DEPS_H" >> src/version.h
echo "#ifndef REDSTASH_VERSION" >> src/version.h
echo "#define REDSTASH_VERSION \"`cat version`\"" >> src/version.h
...
g++ -x c++ - -o $TMPDIR/redstash_build_test.$$ 2>/dev/null <<EOF
