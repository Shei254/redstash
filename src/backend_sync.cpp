...
BackendSync::BackendSync(REDSTASHImpl *redstash, int sync_speed){
	thread_quit = false;
...
void* BackendSync::_run_thread(void *arg){
	...
	REDSTASHImpl *redstash = (REDSTASHImpl *)backend->redstash;
	BinlogQueue *logs = redstash->binlogs;
...
#define NOOP_IDLES			(3000/TICK_INTERVAL_MS)
	...
	while(!backend->thread_quit){
		...
		if(client.status == Client::OUT_OF_SYNC){
			...
			// redstash doesn't do anything, let people interfere, normally, people
...
int BackendSync::Client::copy(){
	if(this->iter == NULL){
		...
		this->iter = backend->redstash->iterator(key, "", -1);
...
int BackendSync::Client::sync(BinlogQueue *logs){
	...
	switch(log.cmd()){
		...
		case BinlogCommand::ZSET:
			ret = backend->redstash->raw_get(log.key(), &val);
			if(ret == -1){
