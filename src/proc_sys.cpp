/*
Copyright (c) 2012-2018 The REDSTASH Authors. All rights reserved.
...
int proc_dbsize(NetworkServer *net, Link *link, const Request &req, Response *resp){
	REDSTASHServer *serv = (REDSTASHServer *)net->data;
	uint64_t size = serv->redstash->size();
	resp->push_back("ok");
...
int proc_version(NetworkServer *net, Link *link, const Request &req, Response *resp){
	resp->push_back("ok");
	resp->push_back(REDSTASH_VERSION);
	return 0;
}
...
int proc_info(NetworkServer *net, Link *link, const Request &req, Response *resp){
	REDSTASHServer *serv = (REDSTASHServer *)net->data;
	resp->push_back("ok");
	resp->push_back("redstash-server");
	...
	{
		std::string s = serv->redstash->binlogs->stats();
	...
	if(req.size() == 1 || req[1] == "range"){
		...
		int ret = serv->redstash->key_range(&tmp);
		if(ret == 0){
			char buf[512];
	...
	if(req.size() == 1 || req[1] == "leveldb"){
		std::vector<std::string> tmp = serv->redstash->info();
