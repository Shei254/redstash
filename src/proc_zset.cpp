...
int proc_zexists(NetworkServer *net, Link *link, const Request &req, Response *resp){
	REDSTASHServer *serv = (REDSTASHServer *)net->data;
	CHECK_NUM_PARAMS(3);

	const Bytes &name = req[1];
	const Bytes &key = req[2];
	std::string val;
	int ret = serv->redstash->zget(name, key, &val);
	resp->reply_bool(ret);
	return 0;
}
...
int proc_multi_zexists(NetworkServer *net, Link *link, const Request &req, Response *resp){
	...
	for(Request::const_iterator it=req.begin()+2; it!=req.end(); it++){
		const Bytes &key = *it;
		int64_t ret = serv->redstash->zget(name, key, &val);
...
int proc_multi_zsize(NetworkServer *net, Link *link, const Request &req, Response *resp){
	...
	for(Request::const_iterator it=req.begin()+1; it!=req.end(); it++){
		const Bytes &key = *it;
		int64_t ret = serv->redstash->zsize(key);
		resp->push_back(key.String());
		if(ret == -1){
			resp->push_back("-1");
		}else{
...
int proc_multi_zset(NetworkServer *net, Link *link, const Request &req, Response *resp){
	...
	if(req.size() < 4 || req.size() % 2 != 0){
		...
		for(; it != req.end(); it += 2){
			...
			int ret = serv->redstash->zset(name, key, val);
			if(ret == -1){
...
int proc_multi_zdel(NetworkServer *net, Link *link, const Request &req, Response *resp){
	...
	for(; it != req.end(); it += 1){
		const Bytes &key = *it;
		int ret = serv->redstash->zdel(name, key);
		if(ret == -1){
			resp->push_back("error");
			return 0;
		}else{
...
int proc_multi_zget(NetworkServer *net, Link *link, const Request &req, Response *resp){
	...
	for(; it!=req.end(); it+=1){
		const Bytes &key = *it;
		std::string score;
		int ret = serv->redstash->zget(name, key, &score);
		if(ret == 1){
			resp->push_back(key.String());
...
int proc_zset(NetworkServer *net, Link *link, const Request &req, Response *resp){
	...
	int ret = serv->redstash->zset(req[1], req[2], req[3]);
...
int proc_zsize(NetworkServer *net, Link *link, const Request &req, Response *resp){
	...
	int64_t ret = serv->redstash->zsize(req[1]);
	resp->reply_int(ret, ret);
	return 0;
}
...
int proc_zget(NetworkServer *net, Link *link, const Request &req, Response *resp){
	...
	std::string score;
	int ret = serv->redstash->zget(req[1], req[2], &score);
...
int proc_zdel(NetworkServer *net, Link *link, const Request &req, Response *resp){
	...
	int ret = serv->redstash->zdel(req[1], req[2]);
	resp->reply_bool(ret);
	return 0;
}
...
int proc_zrank(NetworkServer *net, Link *link, const Request &req, Response *resp){
	...
	int64_t ret = serv->redstash->zrank(req[1], req[2]);
	if(ret == -1){
		resp->add("not_found");
	}else{
		resp->reply_int(ret, ret);
...
int proc_zrrank(NetworkServer *net, Link *link, const Request &req, Response *resp){
	...
	int64_t ret = serv->redstash->zrrank(req[1], req[2]);
	if(ret == -1){
		resp->add("not_found");
	}else{
		resp->reply_int(ret, ret);
...
int proc_zrange(NetworkServer *net, Link *link, const Request &req, Response *resp){
	...
	ZIterator *it = serv->redstash->zrange(req[1], offset, limit);
	resp->push_back("ok");
	while(it->next()){
...
int proc_zrrange(NetworkServer *net, Link *link, const Request &req, Response *resp){
	...
	uint64_t offset = req[2].Uint64();
	uint64_t limit = req[3].Uint64();
	ZIterator *it = serv->redstash->zrrange(req[1], offset, limit);
	resp->push_back("ok");
	while(it->next()){
		resp->push_back(it->key);
		resp->push_back(it->score);
...
int proc_redis_zrange(NetworkServer *net, Link *link, const Request &req, Response *resp){
	...
	uint64_t limit;
	if(start < 0 || end < 0){
		int64_t size = serv->redstash->zsize(req[1]);
		if(start < 0){
			start = size + start;
			if(start < 0){
				start = 0;
	...
	ZIterator *it = serv->redstash->zrange(req[1], start, limit);
	resp->push_back("ok");
	while(it->next()){
		resp->push_back(it->key);
		resp->push_back(it->score);
...
int proc_redis_zrrange(NetworkServer *net, Link *link, const Request &req, Response *resp){
	...
	ZIterator *it = serv->redstash->zrrange(req[1], start, limit);
	resp->push_back("ok");
	while(it->next()){
		resp->push_back(it->key);
		resp->push_back(it->score);
...
int proc_zclear(NetworkServer *net, Link *link, const Request &req, Response *resp){
	...
	while(1){
		ZIterator *it = serv->redstash->zscan(name, key_start, score_start, "", 1000);
		int num = 0;
		while(it->next()){
			key_start = it->key;
			score_start = it->score;
			int ret = serv->redstash->zdel(name, key_start);
			if(ret == -1){
				resp->push_back("error");
				delete it;
				return 0;
...
int proc_zscan(NetworkServer *net, Link *link, const Request &req, Response *resp){
	...
	ZIterator *it = serv->redstash->zscan(req[1], req[2], req[3], req[4], limit);
	if(offset > 0){
		it->skip(offset);
	}
	resp->push_back("ok");
...
int proc_zrscan(NetworkServer *net, Link *link, const Request &req, Response *resp){
	...
	ZIterator *it = serv->redstash->zrscan(req[1], req[2], req[3], req[4], limit);
	if(offset > 0){
		it->skip(offset);
	}
	resp->push_back("ok");
...
int proc_zlist(NetworkServer *net, Link *link, const Request &req, Response *resp){
	...
	int ret = serv->redstash->zlist(req[1], req[2], limit, &list);
...
int proc_zrlist(NetworkServer *net, Link *link, const Request &req, Response *resp){
	...
	uint64_t limit = req[3].Uint64();
	std::vector<std::string> list;
	int ret = serv->redstash->zrlist(req[1], req[2], limit, &list);
	resp->reply_list(ret, list);
	return 0;
}

// dir := +1|-1
static int _zincr(REDSTASH *redstash, const Request &req, Response *resp, int dir){
	CHECK_NUM_PARAMS(3);
	...
	int64_t new_val;
	int ret = redstash->zincr(req[1], req[2], dir * by, &new_val);
...
int proc_zincr(NetworkServer *net, Link *link, const Request &req, Response *resp){
	REDSTASHServer *serv = (REDSTASHServer *)net->data;
	return _zincr(serv->redstash, req, resp, 1);
}
...
int proc_zcount(NetworkServer *net, Link *link, const Request &req, Response *resp){
	...
	int64_t count = 0;
	ZIterator *it = serv->redstash->zscan(req[1], "", req[2], req[3], -1);
	while(it->next()){
		count ++;
	}
	delete it;
...
int proc_zremrangebyscore(NetworkServer *net, Link *link, const Request &req, Response *resp){
	...
	while(it->next()){
		count ++;
		int ret = serv->redstash->zdel(req[1], it->key);
		if(ret == -1){
			delete it;
			resp->push_back("error");
			return 0;
...
int proc_zremrangebyrank(NetworkServer *net, Link *link, const Request &req, Response *resp){
	...
	ZIterator *it = serv->redstash->zrange(req[1], start, end - start + 1);
	int64_t count = 0;
	while(it->next()){
		count ++;
		int ret = serv->redstash->zdel(req[1], it->key);
...
static inline
void zpop(ZIterator *it, REDSTASHServer *serv, const Bytes &name, Response *resp){
	resp->push_back("ok");
	while(it->next()){
		int ret = serv->redstash->zdel(name, it->key);
		if(ret == 1){
			resp->add(it->key);
			resp->add(it->score);
		}
...
int proc_zpop_front(NetworkServer *net, Link *link, const Request &req, Response *resp){
	...
	ZIterator *it = serv->redstash->zscan(name, "", "", "", limit);
...
int proc_zpop_back(NetworkServer *net, Link *link, const Request &req, Response *resp){
	...
	const Bytes &name = req[1];
	uint64_t limit = req[2].Uint64();
	ZIterator *it = serv->redstash->zrscan(name, "", "", "", limit);
	zpop(it, serv, name, resp);
	delete it;
	return 0;
}
...
int proc_zfix(NetworkServer *net, Link *link, const Request &req, Response *resp){
	...
	const Bytes &name = req[1];
	int64_t ret = serv->redstash->zfix(name);
	resp->reply_int(ret, ret);
	return 0;
}
