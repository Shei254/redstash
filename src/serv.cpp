...
void REDSTASHServer::reg_procs(NetworkServer *net){
	REG_PROC(get, "rt");
	REG_PROC(set, "wt");
	REG_PROC(del, "wt");
	REG_PROC(setx, "wt");
...
REDSTASHServer::REDSTASHServer(REDSTASH *redstash, REDSTASH *meta, const Config &conf, NetworkServer *net){
	this->redstash = (REDSTASHImpl *)redstash;
	this->meta = meta;
	...
	backend_dump = new BackendDump(this->redstash);
	backend_sync = new BackendSync(this->redstash, sync_speed);
	expiration = new ExpirationHandler(this->redstash);
...
REDSTASHServer::~REDSTASHServer(){
	...
	log_debug("REDSTASHServer finalized");
}

int REDSTASHServer::slaveof(const std::string &id, const std::string &host, int port, const std::string &auth, uint64_t last_seq, const std::string &last_key, bool is_mirror, int recv_timeout){
	Slave *slave = new Slave(redstash, meta, host.c_str(), port, is_mirror);
	if(!id.empty()){
		slave->set_id(id);
	}
	if(recv_timeout > 0){
...
int REDSTASHServer::set_kv_range(const std::string &start, const std::string &end){
...
int REDSTASHServer::get_kv_range(std::string *start, std::string *end){
...
bool REDSTASHServer::in_kv_range(const Bytes &key){
...
bool REDSTASHServer::in_kv_range(const std::string &key){
	if((this->kv_range_s.size() && this->kv_range_s >= key)
		|| (this->kv_range_e.size() && this->kv_range_e < key))
	{
		return false;
