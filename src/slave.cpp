...
Slave::Slave(REDSTASH *redstash, REDSTASH *meta, const char *ip, int port, bool is_mirror){
	thread_quit = false;
	this->recv_timeout = 30;
	this->redstash = redstash;
	this->meta = meta;
		...
		case BinlogCommand::KSET:
			{
				...
				if(redstash->set(key, req[1], log_type) == -1){
					return -1;
				}
			}
			break;
		case BinlogCommand::KDEL:
			{
				...
				if(redstash->del(key, log_type) == -1){
					return -1;
				}
			}
			break;
		case BinlogCommand::HSET:
			{
				...
				if(redstash->hset(name, key, req[1], log_type) == -1){
					return -1;
				}
			}
			break;
		case BinlogCommand::HDEL:
			{
				...
				log_trace("hdel %s %s",
					hexmem(name.data(), name.size()).c_str(),
					hexmem(key.data(), key.size()).c_str());
				if(redstash->hdel(name, key, log_type) == -1){
					return -1;
				}
			}
			break;
		case BinlogCommand::ZSET:
			{
				...
				log_trace("zset %s %s",
					hexmem(name.data(), name.size()).c_str(),
					hexmem(key.data(), key.size()).c_str());
				if(redstash->zset(name, key, req[1], log_type) == -1){
					return -1;
				}
			}
			break;
		case BinlogCommand::ZDEL:
			{
				...
				log_trace("zdel %s %s",
					hexmem(name.data(), name.size()).c_str(),
					hexmem(key.data(), key.size()).c_str());
				if(redstash->zdel(name, key, log_type) == -1){
					return -1;
				}
			}
			break;
		...
		case BinlogCommand::QPOP_FRONT:
			{
				...
				if(log.cmd() == BinlogCommand::QPOP_BACK){
					...
					ret = redstash->qpop_back(name, &tmp, log_type);
				}else{
					log_trace("qpop_front %s", hexmem(name.data(), name.size()).c_str());
					ret = redstash->qpop_front(name, &tmp, log_type);
				}
				if(ret == -1){
					return -1;
				}
