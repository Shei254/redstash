...
REDSTASH* REDSTASH::open(const Options &opt, const std::string &dir){
	REDSTASHImpl *redstash = new REDSTASHImpl();
	redstash->options.max_file_size = 32 * 1048576; // leveldb 1.20
	redstash->options.create_if_missing = true;
	redstash->options.max_open_files = opt.max_open_files;
	redstash->options.filter_policy = leveldb::NewBloomFilterPolicy(10);
	redstash->options.block_cache = leveldb::NewLRUCache(opt.cache_size * 1048576);
	redstash->options.block_size = opt.block_size * 1024;
	redstash->options.write_buffer_size = opt.write_buffer_size * 1024 * 1024;
	redstash->options.compaction_speed = opt.compaction_speed;
	if(opt.compression == "yes"){
		redstash->options.compression = leveldb::kSnappyCompression;
	}else{
		redstash->options.compression = leveldb::kNoCompression;
	}

	leveldb::Status status;

	status = leveldb::DB::Open(redstash->options, dir, &redstash->ldb);
	if(!status.ok()){
		log_error("open db failed: %s", status.ToString().c_str());
		goto err;
	}
	redstash->binlogs = new BinlogQueue(redstash->ldb, opt.binlog, opt.binlog_capacity);
...
Iterator* REDSTASHImpl::iterator(const std::string &start, const std::string &end, uint64_t limit){
	leveldb::Iterator *it;
	leveldb::ReadOptions iterate_options;
	iterate_options.fill_cache = false;
	it = ldb->NewIterator(iterate_options);
...
Iterator* REDSTASHImpl::rev_iterator(const std::string &start, const std::string &end, uint64_t limit){
	leveldb::Iterator *it;
	leveldb::ReadOptions iterate_options;
	iterate_options.fill_cache = false;
	it = ldb->NewIterator(iterate_options);
...
int REDSTASHImpl::raw_set(const Bytes &key, const Bytes &val){
	leveldb::WriteOptions write_opts;
...
int REDSTASHImpl::raw_del(const Bytes &key){
	leveldb::WriteOptions write_opts;
...
int REDSTASHImpl::raw_get(const Bytes &key, std::string *val){
...
std::vector<std::string> REDSTASHImpl::info(){
...
void REDSTASHImpl::compact(){
	ldb->CompactRange(NULL, NULL);
}

int REDSTASHImpl::key_range(std::vector<std::string> *keys){
	int ret = 0;
