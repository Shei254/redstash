...
static int hset_one(REDSTASHImpl *redstash, const Bytes &name, const Bytes &key, const Bytes &val, char log_type);
static int hdel_one(REDSTASHImpl *redstash, const Bytes &name, const Bytes &key, char log_type);
static int incr_hsize(REDSTASHImpl *redstash, const Bytes &name, int64_t incr);

/**
 * @return -1: error, 0: item updated, 1: new item inserted
 */
int REDSTASHImpl::hset(const Bytes &name, const Bytes &key, const Bytes &val, char log_type){
...
int REDSTASHImpl::hdel(const Bytes &name, const Bytes &key, char log_type){
	Transaction trans(binlogs);
...
int REDSTASHImpl::hincr(const Bytes &name, const Bytes &key, int64_t by, int64_t *new_val, char log_type){
	Transaction trans(binlogs);
...
int64_t REDSTASHImpl::hsize(const Bytes &name){
...
int64_t REDSTASHImpl::hclear(const Bytes &name){
	int64_t count = 0;
	std::string start;
	while(1){
...
int REDSTASHImpl::hget(const Bytes &name, const Bytes &key, std::string *val){
...
HIterator* REDSTASHImpl::hscan(const Bytes &name, const Bytes &start, const Bytes &end, uint64_t limit){
...
HIterator* REDSTASHImpl::hrscan(const Bytes &name, const Bytes &start, const Bytes &end, uint64_t limit){
	std::string key_start, key_end;
...
int REDSTASHImpl::hlist(const Bytes &name_s, const Bytes &name_e, uint64_t limit,
...
int REDSTASHImpl::hrlist(const Bytes &name_s, const Bytes &name_e, uint64_t limit,
		std::vector<std::string> *list){
	std::string start;
	std::string end;
...
static int hset_one(REDSTASHImpl *redstash, const Bytes &name, const Bytes &key, const Bytes &val, char log_type){
	...
	int ret = 0;
	std::string dbval;
	if(redstash->hget(name, key, &dbval) == 0){ // not found
		std::string hkey = encode_hash_key(name, key);
		redstash->binlogs->Put(hkey, slice(val));
		redstash->binlogs->add_log(log_type, BinlogCommand::HSET, hkey);
		ret = 1;
	}else{
		if(dbval != val){
			std::string hkey = encode_hash_key(name, key);
...
static int hdel_one(REDSTASHImpl *redstash, const Bytes &name, const Bytes &key, char log_type){
	...
	std::string dbval;
	if(redstash->hget(name, key, &dbval) == 0){
		return 0;
	}

	std::string hkey = encode_hash_key(name, key);
	redstash->binlogs->Delete(hkey);
	redstash->binlogs->add_log(log_type, BinlogCommand::HDEL, hkey);
	
	return 1;
}

static int incr_hsize(REDSTASHImpl *redstash, const Bytes &name, int64_t incr){
	int64_t size = redstash->hsize(name);
	size += incr;
	std::string size_key = encode_hsize_key(name);
	if(size == 0){
...
int64_t REDSTASHImpl::hfix(const Bytes &name){
	Transaction trans(binlogs);
