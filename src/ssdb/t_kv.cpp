...
int REDSTASHImpl::multi_set(const std::vector<Bytes> &kvs, int offset, char log_type){
...
int REDSTASHImpl::multi_del(const std::vector<Bytes> &keys, int offset, char log_type){
	Transaction trans(binlogs);
...
int REDSTASHImpl::set(const Bytes &key, const Bytes &val, char log_type){
	if(key.empty()){
		log_error("empty key!");
		//return -1;
		return 0;
...
int REDSTASHImpl::setnx(const Bytes &key, const Bytes &val, char log_type){
	if(key.empty()){
		log_error("empty key!");
		//return -1;
		return 0;
...
int REDSTASHImpl::getset(const Bytes &key, std::string *val, const Bytes &newval, char log_type){
	if(key.empty()){
		log_error("empty key!");
		//return -1;
		return 0;
...
int REDSTASHImpl::del(const Bytes &key, char log_type){
	if(key.size() > REDSTASH_KEY_LEN_MAX ){
		log_error("name too long! %s", hexmem(key.data(), key.size()).c_str());
		return -1;
	}
...
int REDSTASHImpl::incr(const Bytes &key, int64_t by, int64_t *new_val, char log_type){
	if(key.size() > REDSTASH_KEY_LEN_MAX ){
		log_error("name too long! %s", hexmem(key.data(), key.size()).c_str());
		return -1;
	}
...
int REDSTASHImpl::get(const Bytes &key, std::string *val){
	std::string buf = encode_kv_key(key);
...
KIterator* REDSTASHImpl::scan(const Bytes &start, const Bytes &end, uint64_t limit){
...
KIterator* REDSTASHImpl::rscan(const Bytes &start, const Bytes &end, uint64_t limit){
	std::string key_start, key_end;
...
int REDSTASHImpl::setbit(const Bytes &key, int bitoffset, int on, char log_type){
	if(key.empty()){
		log_error("empty key!");
		return 0;
	}
...
int REDSTASHImpl::getbit(const Bytes &key, int bitoffset){
	std::string val;
	int ret = this->get(key, &val);
	if(ret == -1){
		return -1;
