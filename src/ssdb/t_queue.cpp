...
static int qdel_one(REDSTASHImpl *redstash, const Bytes &name, uint64_t seq){
	std::string key = encode_qitem_key(name, seq);
	leveldb::Status s;

	redstash->binlogs->Delete(key);
	return 0;
}

static int qset_one(REDSTASHImpl *redstash, const Bytes &name, uint64_t seq, const Bytes &item){
	...
	redstash->binlogs->Put(key, slice(item));
	return 0;
}

static int64_t incr_qsize(REDSTASHImpl *redstash, const Bytes &name, int64_t incr){
	int64_t size = redstash->qsize(name);
	if(size == -1){
		return -1;
	}
	size += incr;
	if(size <= 0){
		redstash->binlogs->Delete(encode_qsize_key(name));
		qdel_one(redstash, name, QFRONT_SEQ);
		qdel_one(redstash, name, QBACK_SEQ);
	}else{
		redstash->binlogs->Put(encode_qsize_key(name), leveldb::Slice((char *)&size, sizeof(size)));
	}
	return size;
}
...
int64_t REDSTASHImpl::qsize(const Bytes &name){
...
int REDSTASHImpl::qfront(const Bytes &name, std::string *item){
	int ret = 0;
	uint64_t seq;
...
// @return 0: empty queue, 1: item peeked, -1: error
int REDSTASHImpl::qback(const Bytes &name, std::string *item){
	int ret = 0;
	uint64_t seq;
...
int REDSTASHImpl::qset_by_seq(const Bytes &name, uint64_t seq, const Bytes &item, char log_type){
...
int REDSTASHImpl::qset(const Bytes &name, int64_t index, const Bytes &item, char log_type){
	Transaction trans(binlogs);
	int64_t size = this->qsize(name);
	if(size == -1){
		return -1;
...
int64_t REDSTASHImpl::_qpush(const Bytes &name, const Bytes &item, uint64_t front_or_back_seq, char log_type){
	Transaction trans(binlogs);
...
int64_t REDSTASHImpl::qpush_front(const Bytes &name, const Bytes &item, char log_type){
	return _qpush(name, item, QFRONT_SEQ, log_type);
}

int64_t REDSTASHImpl::qpush_back(const Bytes &name, const Bytes &item, char log_type){
	return _qpush(name, item, QBACK_SEQ, log_type);
}

int REDSTASHImpl::_qpop(const Bytes &name, std::string *item, uint64_t front_or_back_seq, char log_type){
	Transaction trans(binlogs);
...
int REDSTASHImpl::qpop_front(const Bytes &name, std::string *item, char log_type){
	return _qpop(name, item, QFRONT_SEQ, log_type);
}

int REDSTASHImpl::qpop_back(const Bytes &name, std::string *item, char log_type){
...
int REDSTASHImpl::qlist(const Bytes &name_s, const Bytes &name_e, uint64_t limit,
...
int REDSTASHImpl::qrlist(const Bytes &name_s, const Bytes &name_e, uint64_t limit,
		std::vector<std::string> *list){
	std::string start;
	std::string end;
...
int REDSTASHImpl::qfix(const Bytes &name){
	Transaction trans(binlogs);
...
int REDSTASHImpl::qslice(const Bytes &name, int64_t begin, int64_t end,
		std::vector<std::string> *list)
{
	int ret;
...
int REDSTASHImpl::qget(const Bytes &name, int64_t index, std::string *item){
	int ret;
	uint64_t seq;
	if(index >= 0){
		ret = qget_uint64(this->ldb, name, QFRONT_SEQ, &seq);
