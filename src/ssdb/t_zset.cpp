...
static const char *REDSTASH_SCORE_MIN		= "-9223372036854775808";
static const char *REDSTASH_SCORE_MAX		= "+9223372036854775807";

static int zset_one(REDSTASHImpl *redstash, const Bytes &name, const Bytes &key, const Bytes &score, char log_type);
static int zdel_one(REDSTASHImpl *redstash, const Bytes &name, const Bytes &key, char log_type);
static int incr_zsize(REDSTASHImpl *redstash, const Bytes &name, int64_t incr);

/**
 * @return -1: error, 0: item updated, 1: new item inserted
 */
int REDSTASHImpl::zset(const Bytes &name, const Bytes &key, const Bytes &score, char log_type){
...
int REDSTASHImpl::zdel(const Bytes &name, const Bytes &key, char log_type){
	Transaction trans(binlogs);
...
int REDSTASHImpl::zincr(const Bytes &name, const Bytes &key, int64_t by, int64_t *new_val, char log_type){
	Transaction trans(binlogs);
...
int64_t REDSTASHImpl::zsize(const Bytes &name){
...
int REDSTASHImpl::zget(const Bytes &name, const Bytes &key, std::string *score){
	...
	uint64_t limit, Iterator::Direction direction)
{
	if(direction == Iterator::FORWARD){
		...
		if(score_start.empty()){
			start = encode_zscore_key(name, key_start, REDSTASH_SCORE_MIN);
		}else{
			start = encode_zscore_key(name, key_start, score_start);
		}
		if(score_end.empty()){
			end = encode_zscore_key(name, "\xff", REDSTASH_SCORE_MAX);
		}else{
			end = encode_zscore_key(name, "\xff", score_end);
		}
		return new ZIterator(redstash->iterator(start, end, limit), name);
	}else{
		std::string start, end;
		if(score_start.empty()){
			start = encode_zscore_key(name, key_start, REDSTASH_SCORE_MAX);
		}else{
			if(key_start.empty()){
		...
		if(score_end.empty()){
			end = encode_zscore_key(name, "", REDSTASH_SCORE_MIN);
		}else{
			end = encode_zscore_key(name, "", score_end);
		}
		return new ZIterator(redstash->rev_iterator(start, end, limit), name);
	}
}

int64_t REDSTASHImpl::zrank(const Bytes &name, const Bytes &key){
...
int64_t REDSTASHImpl::zrrank(const Bytes &name, const Bytes &key){
...
ZIterator* REDSTASHImpl::zrange(const Bytes &name, uint64_t offset, uint64_t limit){
...
ZIterator* REDSTASHImpl::zrrange(const Bytes &name, uint64_t offset, uint64_t limit){
	if(offset + limit > limit){
		limit = offset + limit;
	}
...
ZIterator* REDSTASHImpl::zscan(const Bytes &name, const Bytes &key,
...
ZIterator* REDSTASHImpl::zrscan(const Bytes &name, const Bytes &key,
		const Bytes &score_start, const Bytes &score_end, uint64_t limit)
{
	std::string score;
	// if only key is specified, load its value
...
int REDSTASHImpl::zlist(const Bytes &name_s, const Bytes &name_e, uint64_t limit,
...
int REDSTASHImpl::zrlist(const Bytes &name_s, const Bytes &name_e, uint64_t limit,
		std::vector<std::string> *list){
	std::string start;
	std::string end;
...
static int zset_one(REDSTASHImpl *redstash, const Bytes &name, const Bytes &key, const Bytes &score, char log_type){
	...
	if(name.size() > REDSTASH_KEY_LEN_MAX ){
		log_error("name too long!");
		return -1;
	}
	if(key.size() > REDSTASH_KEY_LEN_MAX){
		log_error("key too long!");
		return -1;
	}
	std::string new_score = filter_score(score);
	std::string old_score;
	int found = redstash->zget(name, key, &old_score);
	if(found == 0 || old_score != new_score){
		...
		if(found){
			...
			redstash->binlogs->Delete(k1);
		}

		// add zscore key
		k2 = encode_zscore_key(name, key, new_score);
		redstash->binlogs->Put(k2, "");

		// update zset
		k0 = encode_zset_key(name, key);
		redstash->binlogs->Put(k0, new_score);
		redstash->binlogs->add_log(log_type, BinlogCommand::ZSET, k0);
...
static int zdel_one(REDSTASHImpl *redstash, const Bytes &name, const Bytes &key, char log_type){
	...
	// delete zset
	k0 = encode_zset_key(name, key);
	redstash->binlogs->Delete(k0);
	redstash->binlogs->add_log(log_type, BinlogCommand::ZDEL, k0);

	return 1;
}

static int incr_zsize(REDSTASHImpl *redstash, const Bytes &name, int64_t incr){
	int64_t size = redstash->zsize(name);
	size += incr;
	std::string size_key = encode_zsize_key(name);
	if(size == 0){
		redstash->binlogs->Delete(size_key);
	}else{
		redstash->binlogs->Put(size_key, leveldb::Slice((char *)&size, sizeof(int64_t)));
	}
	return 0;
}

int64_t REDSTASHImpl::zfix(const Bytes &name){
	Transaction trans(binlogs);
	...
	it_start = encode_zscore_key(name, "", REDSTASH_SCORE_MIN);
	it_end = encode_zscore_key(name, "\xff", REDSTASH_SCORE_MAX);
