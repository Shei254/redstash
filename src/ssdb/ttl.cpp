...
ExpirationHandler::ExpirationHandler(REDSTASH *redstash){
...
int ExpirationHandler::set_ttl(const Bytes &key, int64_t ttl){
	...
	int ret = redstash->zset(this->list_name, key, expire_str);
	if(ret == -1){
		return -1;
	}
...
int ExpirationHandler::del_ttl(const Bytes &key){
	...
	if(first_timeout != INT64_MAX){
		...
		redstash->zdel(this->list_name, key);
	}
	return 0;
}
...
int64_t ExpirationHandler::get_ttl(const Bytes &key){
	std::string score;
	if(redstash->zget(this->list_name, key, &score) == 1){
...
void ExpirationHandler::load_expiration_keys_from_db(int num){
	ZIterator *it;
	it = redstash->zscan(this->list_name, "", "", "", num);
	int n = 0;
	while(it->next()){
		n ++;
