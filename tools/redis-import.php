 ...
 *
 * PHP script for importing Redis data into REDSTASH.
 */

function usage(){
	global $argv;
	echo "Usage:\n";
	echo "    php {$argv[0]} redis_host redis_port redis_db redstash_host redstash_port\n";
	echo "\n";
}
...
$redstash = new Redis();
$ret = $redstash->connect($s_host, $s_port);
if($ret === false){
	echo "ERROR: could not connect to REDSTASH server!\n";
	die();
}


echo "\nCopying data from Redis($r_host:{$r_port}[$r_db]) to REDSTASH($s_host, $s_port)...\n";
...
function copy_keys($keys){
	global $redis, $redstash, $count, $total, $entries;
