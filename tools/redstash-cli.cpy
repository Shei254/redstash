...
sys.path.append('../api/python');
sys.path.append('/usr/local/redstash/api/python');
import REDSTASH.*;
...
try{
	import readline;
	import atexit;
	histfile = os.path.expanduser('~/.redstash-cli_history');
...
function welcome(){
	sys.stderr.write('redstash (cli) - redstash command line tool.\n');
	sys.stderr.write('Copyright (c) 2012-2016 redstash.io\n');
	sys.stderr.write('\n');
...
function show_command_help(){
	print '';
	print '# display redstash-server status';
	print '	info';
	...
	print '';
	print 'see http://redstash.io/docs/php/ for commands details';
	print '';
...
function usage(){
	print 'Usage:';
	print '        redstash-cli [-h] <host> [-p] <port>';
	print '';
	print 'Options:';
	print '  -h <host>      redstash server hostname/ip address (default: 127.0.0.1)';
	print '  -p <port>      redstash server port (default: 8888)';
	...
	print 'Examples:';
	print '  redstash-cli';
	print '  redstash-cli 8888';
	print '  redstash-cli 127.0.0.1 8888';
	print '  redstash-cli -h 127.0.0.1 -p 8888';
	print '  redstash-cli -h 127.0.0.1 -p 8888 -a xxxpasswordxxx';
}
...
while(true){
	line = '';
	c = sprintf('redstash %s:%s> ', host, str(port));
	b = sys.stdout;
