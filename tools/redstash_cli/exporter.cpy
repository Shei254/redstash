...
function run(link, args){
	...
	// KV
	ls = new REDSTASH_kv_scan(link);
	ls.set_range(kstart, kend);
	// by default, redstash's iterator is start-exclusive,
	...
	// HASH
	ls = new REDSTASH_hash_list(link);
	ls.set_range(hstart, hend);
	scan = new REDSTASH_hash_scan(link);
	scan.name = ls.key;
	while(scan.next()){
		show_progress();
	...
	// ZSET
	ls = new REDSTASH_zset_list(link);
	ls.set_range(zstart, zend);
	scan = new REDSTASH_zset_scan(link);
	scan.name = ls.key;
	while(scan.next()){
		show_progress();
	...
	// QUEUE
	ls = new REDSTASH_queue_list(link);
	ls.set_range(qstart, qend);
	scan = new REDSTASH_queue_scan(link);
	scan.name = ls.key;
	while(scan.next()){
		show_progress();
