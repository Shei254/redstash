...
redstash::Client *src = NULL;
redstash::Client *dst = NULL;

void welcome(){
	printf("redstash-migrate - REDSTASH server migration tool\n");
	printf("Copyright (c) 2012-2015 redstash.io\n");
	printf("\n");
}
...
void usage(int argc, char **argv){
	printf("Usage:\n"
		...
		"    src_ip    IP addr of the REDSTASH server to move data from, example: 127.0.0.1\n"
		"    src_port  Port number of source REDSTASH server\n"
		"    src_ip    IP addr of the REDSTASH server to move data to, example: 127.0.0.1\n"
		"    dst_port  Port number of destination REDSTASH server\n"
...
redstash::Client* init_client(const std::string &ip, int port){
...
int get_key_range(redstash::Client *client, KeyRange *range){
	const std::vector<std::string>* resp;
...
int set_key_range(redstash::Client *client, const KeyRange &range){
	const std::vector<std::string>* resp;
...
void check_version(redstash::Client *client){
	const std::vector<std::string>* resp;
	resp = client->request("version");
	if(!resp || resp->size() < 2 || resp->at(0) != "ok"){
		fprintf(stderr, "ERROR: redstash-server 1.9.0 or higher is required!\n");
		exit(1);
	}
}
